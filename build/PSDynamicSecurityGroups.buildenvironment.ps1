...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleTags = 'Active Directory,Security Groups,Group,Dynamic,Dynamic Security Groups,Shadow Groups' -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the PSDynamicSecurityGroups.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSDynamicSecurityGroups.buildenvironment.json"
