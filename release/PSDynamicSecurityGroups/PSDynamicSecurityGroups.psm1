...
Function Confirm-DSGDestination {
    ...
    Try {
        Write-Debug "Check that the destination OU exists, otherwise we won't be able to create the Dynamic Security Groups at all"
        Try {
        ...
        Catch {
            If ($_.Exception.GetType().Name -eq "ADIdentityNotFoundException") {
                Write-Error "Skipping sync of '$GroupName', following destination OU does not exist: '$DestOU'"
                return $false
            }
            Else {
              Write-Error "Confirm-DSGDestination - $PSItem"
            }
        }
        Write-Debug "Check that a group with the same sAMAccountName as our destination group does not exist Elsewhere in AD - this attribute must be unique within a the Active Directory."
        $ADGroup = Get-ADGroup -Filter {sAMAccountName -eq $GroupName} -ErrorAction Continue
        Write-Debug "If group already exists ensure it's in the expected OU"
        If($ADGroup -and (([string]([ADSI]"LDAP://$ADGroup").PSBase.Parent.distinguishedName) -ne $DestOU)) {
            Write-Error "Skipping sync of '$GroupName', a group with the same sAMAccountName already exists in a fifferent part of the hierarchy:'$($ADGroup.DistinguishedName)'"
            Return $false
        }
        Return $true
    } Catch {
...
Function Update-DSG {
    ...
    .LINK
        https://github.com/GraficomGroup/PSDynamicSecurityGroup/tree/master/release/1.0.0/docs/Functions/Update-DSG.md
    #>    
    [Cmdletbinding(SupportsShouldProcess)]
    Param (
        [Parameter(Mandatory = $true, HelpMessage = 'The base OU DistinguishedName of to search for objects. Multiples can be specIfied and chained together with a semicolon.')]
